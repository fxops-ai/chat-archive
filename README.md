...
> **⚠️ Important:** `src/utils/constants.js` is the shared utilities module for the
> entire codebase. Despite its name, it contains DOM helpers, scroll handling, clipboard
> utilities, and button-finding logic used by all five platform extractors. When patching
> or replacing this file, always diff against the existing version first — stripping it
> to just constants will break extraction on all platforms.
> See: [v0.2.1 post-mortem](#changelog) for context.
...
**Build discipline:** Always run `build.sh` before committing. The root `content.js`
...
2. Run `build.sh` to rebuild root `content.js`
3. **Remove** Chat Archive from `chrome://extensions/` entirely
4. **Load unpacked** → select the repo root folder
5. Confirm the version number on the extension card matches `manifest.json`
6. Run the ARCHIVETEST conversation on target platform (see architecture doc)
7. Export both JSON and Markdown, validate both files
8. Check browser console for `[Chat Archive]` log messages
